...
require_once( PENCI_AMP_DIR. '/includes/sanitizers/class-amp-allowed-tags-generated.php' );
 ...
 *     by the python script in amp-wp/bin/penci_amp_wp_build.py. See the comment at the top
 ...
 */
class AMP_Tag_And_Attribute_Sanitizer extends Penci_AMP_Base_Sanitizer {
	...
	public function __construct( $dom, $args = array() ) {
		$this->DEFAULT_ARGS = array(
			'penci_amp_allowed_tags' => Penci_AMP_Allowed_Tags_Generated::get_allowed_tags(),
			'penci_amp_globally_allowed_attributes' => Penci_AMP_Allowed_Tags_Generated::get_allowed_attributes(),
			'penci_amp_layout_allowed_attributes' => Penci_AMP_Allowed_Tags_Generated::get_layout_attributes(),
		);

		parent::__construct( $dom, $args );

		// Prepare whitelists
		$this->allowed_tags = $this->args['penci_amp_allowed_tags'];
		$this->globally_allowed_attributes = $this->args['penci_amp_globally_allowed_attributes'];
		$this->layout_allowed_attributes = $this->args['penci_amp_layout_allowed_attributes'];
	}
	...
	private function process_node( $node ) {
		...
		if ( ! $this->is_penci_amp_allowed_tag( $node ) ) {
	...
	private function sanitize_disallowed_attributes_in_node( $node, $attr_spec_list ) {
		...
		foreach ( $node->attributes as $attr_name => $attr_node ) {
			...
			if ( ! $this->is_penci_amp_allowed_attribute( $attr_name, $attr_spec_list ) ) {
		...
		if ( isset( $attr_spec_rule[AMP_Rule_Spec::ALLOWED_PROTOCOL] ) ) {
			if ( $node->hasAttribute( $attr_name ) ) {
				...
				foreach ( $urls_to_test as $url ) {
					...
					if ( $url_scheme = Penci_AMP_WP_Utils::parse_url( $url, PHP_URL_SCHEME ) ) {
	...
	private function check_attr_spec_rule_disallowed_domain( $node, $attr_name, $attr_spec_rule ) {
		if ( isset( $attr_spec_rule[AMP_Rule_Spec::DISALLOWED_DOMAIN] ) &&
			$node->hasAttribute( $attr_name ) ) {
			$attr_value = $node->getAttribute( $attr_name );
			$url_domain = Penci_AMP_WP_Utils::parse_url( $attr_value, PHP_URL_HOST );
			if ( ! empty( $url_domain ) ) {
	 ...
	 */
	private function is_penci_amp_allowed_attribute( $attr_name, $attr_spec_list ) {
	 ...
	 */
	private function is_penci_amp_allowed_tag( $node ) {
